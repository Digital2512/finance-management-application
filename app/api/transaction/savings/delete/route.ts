...
import { deleteSavingsTransaction } from '@/lib/transaction/deleteSavingsTransaction';

export async function POST(request: NextRequest) {
  try {
    const { transactionID } = await request.json(); 
    console.log('Not Deleted Transaction: ' + transactionID); 
    const {result, message, deletedSavingsTransactionID} = await deleteSavingsTransaction(transactionID);

    console.log('Deleted Transaction: ' + deletedSavingsTransactionID);
    ...
    if(result){
      return NextResponse.json({ message: message, deleteTransactionID: deletedSavingsTransactionID }, { status: 200 });
    }else{
