...
import { fetchSavingsTransaction } from "@/lib/transaction/fetchSavingsTransaction";
...
export async function GET(request: NextRequest) {
  console.log('Fetch Transaction Info Request JSON: ', request);
    try {
      ...
      const transactionID = searchParams.get('savingsTransactionID'); // Get transactionID
      ...
      const {result, message, existingTransactionData} = await fetchSavingsTransaction(transactionID);
