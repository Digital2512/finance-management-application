...
import { fetchUserSavingsTransaction } from "@/lib/transaction/fetchUserSavingsTransactions";
...
export async function GET(request: NextRequest) {
    try {
        ...
        if(userID){
            const {result, message, userSavingsTransactionsData} = await fetchUserSavingsTransaction(userID);
            
            console.log('Transaction Info: ', userSavingsTransactionsData);
    
            console.log('----------------------------------------------------------------------------------------------------------------------------------------------------');

            if(result){
                return NextResponse.json({ message: message, userSavingsTransactionsData: userSavingsTransactionsData }, { status: 200 });
              }else{
