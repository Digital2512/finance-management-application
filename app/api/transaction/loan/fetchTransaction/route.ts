...
export async function GET(request: NextRequest) {
  console.log('Fetch Transaction Info Request JSON: ', request);
    try {
      ...
      const transactionID = searchParams.get('loanTransactionID'); // Get transactionID
      ...
      const {result, message, existingTransactionData} = await fetchLoanTransaction(transactionID);
