...
export async function POST(request: NextRequest) {
  try {
    ...
    const {result, message, deletedLoanTransactionID} = await deleteLoanTransaction(transactionID);

    console.log('Deleted Transaction: ' + deletedLoanTransactionID);
    ...
    if(result){
      return NextResponse.json({ message: message, deleteTransactionID: deletedLoanTransactionID }, { status: 200 });
    }else{
