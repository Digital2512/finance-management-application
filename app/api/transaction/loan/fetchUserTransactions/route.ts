...
export async function GET(request: NextRequest) {
    try {
        ...
        if(userID){
            const {result, message, userLoanTransactionsData} = await fetchUserLoanTransaction(userID);
            
            console.log('Transaction Info: ', userLoanTransactionsData);
    
            console.log('----------------------------------------------------------------------------------------------------------------------------------------------------');

            if(result){
                return NextResponse.json({ message: message, userLoanTransactionsData: userLoanTransactionsData }, { status: 200 });
              }else{
