...
import { deleteLoanTransaction } from '@/lib/transaction/deleteLoanTransaction';
import { deleteRepaymentTransaction } from '@/lib/transaction/deleteRepaymentTransaction';

export async function POST(request: NextRequest) {
  try {
    const { transactionID } = await request.json(); 
    console.log('Not Deleted Transaction: ' + transactionID); 
    const {result, message, deletedRepaymentTransactionID} = await deleteRepaymentTransaction(transactionID);

    console.log('Deleted Transaction: ' + deletedRepaymentTransactionID);
    ...
    if(result){
      return NextResponse.json({ message: message, deleteTransactionID: deletedRepaymentTransactionID }, { status: 200 });
    }else{
