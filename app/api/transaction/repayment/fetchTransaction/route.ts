...
import { fetchLoanTransaction } from "@/lib/transaction/fetchLoanTransaction";
import { fetchRepaymentTransaction } from "@/lib/transaction/fetchRepaymentTransaction";
...
export async function GET(request: NextRequest) {
  console.log('Fetch Transaction Info Request JSON: ', request);
    try {
      ...
      const transactionID = searchParams.get('repaymentTransactionID'); // Get transactionID
      ...
      const {result, message, existingTransactionData} = await fetchRepaymentTransaction(transactionID);
