...
const ExpenseDoughnutChart = ({ expenses }: ExpenseDoughnutChartProps) => {

  // Prepare the data for the doughnut chart
  const expenseData = expenses.map(expense => expense.amount);  // Extract amounts
  const expenseLabels = expenses.map(expense => expense.category);  // Extract categories
  ...
  const data = {
    ...
    labels: [...expenseLabels, 'Debt']  // Include "Debt" as a category
  };
