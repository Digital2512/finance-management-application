...
interface SavingssDoughnutChartProps {
  savings: Array<{ category: string, amount: number }>;
}

const SavingsDoughnutChart = ({ savings }: SavingssDoughnutChartProps) => {

  // Prepare the data for the doughnut chart
  const savingsData = savings.map(saving => saving.amount);  // Extract amounts
  const savingsLabels = savings.map(saving => saving.category);  // Extract categories
  ...
  const data = {
    ...
    labels: [...savingsLabels, 'savings']  // Include "savings" as a category
  };
...
export default SavingsDoughnutChart;
