...
export const deleteSavingsTransaction = async ( 
  savingsTransactionID: string, 
    ...
    console.log('Delete Transaction with ID:', { _id: savingsTransactionID }); // Log the incoming data
    ...
    try {
      const existingTransaction = await Savings.findOne({_id: savingsTransactionID});
      if (!existingTransaction) {
        console.log('No record of this transaction is found');
        return { result: false, message: 'Transaction is not in Database' };
      }else{
        const deleteOldTransaction = await Savings.deleteOne({_id: savingsTransactionID})
        ...
        return { result: true, message: 'Transaction deleted', deletedSavingsTransactionID: savingsTransactionID};
      }
  } catch (error) {
