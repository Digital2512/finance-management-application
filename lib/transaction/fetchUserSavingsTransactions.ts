...
export const fetchUserSavingsTransaction = async ( userID: string ) => {
    ...
    try {
        ...
        const userSavingsTransactionsData = await Savings.find({userID: userIDObject});
      if (!userSavingsTransactionsData) {
        ...
        console.log('User Loan Transactions: ', userSavingsTransactionsData);
        return { result: true, message: 'User Loan Transactions fetched', userSavingsTransactionsData: userSavingsTransactionsData};
      }
  } catch (error) {
