...
export const deleteLoanTransaction = async ( 
    ...
    try {
      const existingTransaction = await Loan.findOne({_id: loanTransactionID});
      if (!existingTransaction) {
        console.log('No record of this transaction is found');
        return { result: false, message: 'Transaction is not in Database' };
      }else{
        const deleteOldTransaction = await Loan.deleteOne({_id: loanTransactionID})
        ...
        return { result: true, message: 'Transaction deleted', deletedLoanTransactionID: loanTransactionID};
      }
  } catch (error) {
