...
export const deleteRepaymentTransaction = async ( 
  repaymentTransactionID: string, 
) => {
    console.log('Delete Transaction with ID:', { _id: repaymentTransactionID }); // Log the incoming data
    ...
    try {
      const existingTransaction = await Repayment.findOne({_id: repaymentTransactionID});
      if (!existingTransaction) {
        console.log('No record of this transaction is found');
        return { result: false, message: 'Transaction is not in Database' };
      }else{
        const deleteOldTransaction = await Repayment.deleteOne({_id: repaymentTransactionID})
        ...
        return { result: true, message: 'Transaction deleted', deletedRepaymentTransactionID: repaymentTransactionID};
      }
  } catch (error) {
